...
func generateAccounts(ctx *generatorContext, dl *diskLayer, accMarker []byte) error {
	onAccount := func(key []byte, val []byte, write bool, delete bool) error {
		...
		if acc.Root == types.EmptyRootHash || acc.Root == zeroRoot {
			ctx.removeStorageAt(account)
		} else {
	...
	for {
		...
		exhausted, last, err := dl.generateRange(ctx, id, rawdb.SnapshotAccountPrefix, snapAccount, origin, accountRange, onAccount, nil)
		if err != nil {
			return err // The procedure it aborted, either by external signal or internal error.
		}
